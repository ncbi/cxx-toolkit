---
layout: default
title: The NCBI C++ Toolkit Book
nav: index
---

{{ page.title }}
=========================

National Center for Biotechnology Information (US): Bethesda (MD); 2004-2022.

Contents
--------

-   [Book Information](pages/fm)
-   [Part 1. Overview](pages/part1)
    -   [Introduction to the C++ Toolkit](pages/ch_intro)
    -   [Getting Started](pages/ch_start)
-   [Part 2. Development Framework](pages/part2)
    -   [Retrieve the Source Code (GitHub, Subversion and FTP)](pages/ch_getcode_svn)
    -   [Configure, Build and Use the Toolkit with CMake](pages/ch_cmconfig)
    -   [Configure, Build, and Use the Toolkit with autoconf/PTB](pages/ch_config)
    -   [Working with Makefiles (autoconf/PTB)](pages/ch_build)
    -   [Project Creation and Management (autoconf/PTB)](pages/ch_proj)
    -   [Programming Policies and Guidelines](pages/ch_style)
-   [Part 3. C++ Toolkit Library Reference](pages/part3)
    -   [Portability, Core Functionality and Application Framework](pages/ch_core)
    -   [Logging and Diagnostics](pages/ch_log)
    -   [Networking and IPC](pages/ch_conn)
    -   [Database Access Support](pages/ch_dbapi)
    -   [CGI and Fast-CGI](pages/ch_cgi)
    -   [HTML](pages/ch_html)
    -   [Row-based data processing (TSV, CSV, etc.)](pages/ch_tabular) 
    -   [Data compression (ZIP, GZIP, BZip2, LZO)](pages/ch_compress) 
    -   [Data Serialization (ASN.1, XML, JSON)](pages/ch_ser)
    -   [Biological Sequence Data Model](pages/ch_datamod)
    -   [Biological Object Manager](pages/ch_objmgr)
    -   [Biological Object Manager – Data Loaders](pages/ch_objmgr_dtld)   
    -   [BLAST API](pages/ch_blast)
    -   [Access to NCBI data](pages/ch_dataaccess)
    -   [Biological Sequence Alignment](pages/ch_algoalign)
    -   [GUI and Graphics](pages/ch_gui)
    -   [Using the Boost Unit Test Framework](pages/ch_boost)
-   [Part 4. Wrappers for 3rd-Party Packages](pages/part4)
    -   [XmlWrapp (XML parsing and handling, XSLT, XPath)](pages/ch_xmlwrapp)
    -   [JsonWrapp (JSON parsing and handling)](pages/ch_jsonwrapp)
-   [Part 5. Software](pages/part5)
    -   [Debugging, Exceptions, and Error Handling](pages/ch_debug)
    -   [Distributed Computing](pages/ch_grid)
    -   [Applications](pages/ch_app)
    -   [Examples and Demos](pages/ch_demo)
    -   [C Toolkit Resources for C++ Toolkit Users](pages/ch_res)
-   [Part 6. Help and Support](pages/part6)
    -   [NCBI C++ Toolkit Source Browser](pages/ch_browse)
    -   [Software Development Tools](pages/ch_devtools)
    -   [FAQs, Useful Documentation Links, and Mailing Lists](pages/ch_faq)
-   [Part 7. Library and Applications Configuration](pages/part7)
    -   [Library Configuration](pages/ch_libconfig)
-   [Miscellaneous](pages/misc)
-   [Appendix - Books and Styles](pages/appendix)


