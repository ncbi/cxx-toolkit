...
| Maximum queue load. Essentially, a maximum number of pending requests and replies combined that a queue can hold. Adding a request to a maximally loaded queue will block until a timeout is reached or the load is reduced. Zero means no limit on queue load.   | **`[PSG]`**<br/>**`max_queue_load`**<br/><br/>**`NCBI_CONFIG__PSG__MAX_QUEUE_LOAD`**[<sup>a</sup>](#ch_libconfig.TF.psg_client_library.1)                                                            | integer \>= 0                                                             | 100                        |
