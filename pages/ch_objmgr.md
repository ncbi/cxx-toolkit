...
Data loader is a link between in-process data storage and remote, out-of process data source. Data loaders, their configuration and usage are described in [Data Loaders chapter](ch_objmgr_dtld).
...
Sometimes the exact set of required data loaders may be unknown at compile time. In this case dynamic loading can be used: the Object Manager takes a loader name and uses [Plugin Manager](http://intranet.ncbi.nlm.nih.gov/ieb/ToolBox/CPP_DOC/lxr/ident?i=CPluginManager&d=) to find a DLL named ncbi_xloader_\<loader name\> and create an instance of data loader from that DLL. The DLL must exist in a standard DLL search path for the Plugin Manager to be able to find it.
