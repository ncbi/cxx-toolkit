...
[CMake](https://cmake.org) is an open-source, cross-platform family of tools designed to build, test and package software. It uses compiler-independent configuration files to generate native makefiles and workspaces that can be used in a variety of compiler environments.
...
-   *--with-projects=*”FILE” – build projects listed in FILE. This is either a Project List File, or a list of subdirectories of *src* directory. Each entry is a regular expression (see [CMake documentation](https://cmake.org/cmake/help/v3.14/command/string.html#regex-specification) for details).

-   *--with-targets=*”NAMES” – lists targets to build. Again, each entry is a regular expression.

-   *--with-tags=*”TAGS” – build targets with the listed tags only. 
