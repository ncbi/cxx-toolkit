...
-   [Quick Start](#ch_start.quick_start)
     ...
     -   [First Steps](#ch_start.start_101)
...
The NCBI C++ Toolkit supports two build systems: one based on [GNU autoconf](https://www.gnu.org/software/autoconf/) and one based on [CMake](https://cmake.org).
The Autoconf build system is being phased out, and may eventually be removed. Also, the Toolkit uses a number of 3rd party open source packages.
There is a pretty flexible mechanism to "find" them during configuration, but we recommend using [Conan](https://conan.io) package manager.
...
<a name="ch_start.start_101_"></a>

### First Steps.

Once you have the Toolkit source code, let us try something simple. Run the following command:

    cmake-configure --with-projects=corelib

which means *configure build of projects found in `src/corelib` subdirectory*.
...
So, when the script ends, you will find something like `CMake-GCC1320-Debug` directory. It has the following subdirectories:

- bin   - the output directory of runtime build targets (applications),
- build - build system files generated by CMake,
- check - test results generated by NCBI test framework,
- inc   - generated header files, which describe the build platform,
- lib   - the output directory of library build targets,
- status - the status (found or not found) of various features required by the Toolkit,
- testing - test results generated by CTest framework.

Build the tree and run tests:

    cd CMake-GCC1320-Debug/build
    cmake --build . -j
    ctest . -j

Observe test results in `CMake-GCC1320-Debug/testing/corelib/test` directory.

Now let us try to build something more interesting - [blast\_demo](https://github.com/ncbi/ncbi-cxx-toolkit-public/blob/main/src/sample/app/blast/CMakeLists.blast_demo.app.txt)
for example:

    cmake-configure --with-conan --without-debug --with-targets=blast_demo
    cd CMake-GCC1320-Release/build/
    cmake --build . -j

This time `cmake-configure` runs much longer. It begins with installation of external Conan packages. This takes time, but once they are in
Conan cache, the subsequent runs will be much faster. When the build finished you will find two executables in `CMake-GCC1320-Release/bin` directory:
`blast_demo` and `remote_blast_demo`. This is because `--with-targets` argument is interpreted as a list of 
[regular expressions](https://cmake.org/cmake/help/latest/command/string.html#regex-specification).
To build `blast_demo` only, we should have specified `--with-targets=^blast_demo$`.

More details on how to configure the build can be found [here](ch_cmconfig.html#_Configure).
...
-   [app/netschedule](https://www.ncbi.nlm.nih.gov/IEB/ToolBox/CPP_DOC/lxr/source/src/sample/app/netschedule/) - Creates an NCBI [GRID](https://confluence.ncbi.nlm.nih.gov/display/CT/GRID) application that uses [NetSchedule](https://ncbi.github.io/cxx-toolkit/pages/misc).
