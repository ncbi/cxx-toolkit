...
This chapter describes in detail how to configure, build, and use the NCBI C++ Toolkit (or selected components of it) on supported platforms. See the [Getting Started](ch_start.html#ch_start.basic_install) chapter for a general overview of the process. A list of all supported platforms can be seen [here](https://ncbi.github.io/cxx-toolkit/pages/release_notes#release_notes.Platforms_OSs__compi).
...
3.  For MSVC, the **`-projtag`** option of the **`PTB_FLAGS`** macro in the `compilers\vs2019\static\build\UtilityProjects\configure._` file for non-interactive configuring, or the same option in the `configure_dialog._` file for interactive configuring.
...
`compilers\vs2019\static\build\ncbi_cpp.sln`

`compilers\vs2019\dll\build\ncbi_cpp.sln`

`compilers\vs2019\static\build\gui\ncbi_gui.sln`

`compilers\vs2019\dll\build\gui\ncbi_gui.sln`

`compilers\vs2019\dll\build\gbench\ncbi_gbench.sln`
...
There is a template solution, `compilers\vs2019\user\build\ncbi_user.sln`, that should help you build a customized solution. The project list for this solution is in `scripts\projects\ncbi_user.lst`

***Note:*** Do not use this solution directly. Instead, make a new solution based on the template:

1.  Make copies of the `compilers\vs2019\user\` subtree and the `scripts\projects\ncbi_user.lst` file (keep the copies in the same folders as the originals).

2.  Rename the subtree, solution file, and project list file appropriately, for example to `compilers\vs2019\project_name\, compilers\vs2019\project_name\build\project_name.sln`, and `scripts\projects\project_name.lst`.

3.  In the folder `compilers\vs2019\project_name\build\UtilityProjects\`, use a text editor to edit `_CONFIGURE_.vcproj`, and `_CONFIGURE_DIALOG_.vcproj`. Change all instances of "`ncbi_user`" to "`project_name`".
...
6.  Your custom solution can now be built. Open the solution file `compilers\vs2019\project_name\build\project_name.sln`, [configure](#ch_config.Configuring_with_Vis), and [build](#ch_config.Building_with_Visual).

Note that the project directory, `vs2019`, may be different for your version of Visual C++.
...
`compilers\vs2019\{static|dll}\bin\<config_name>`

`compilers\vs2019\{static|dll}\lib\<config_name>`
...
For example, if the Toolkit is built in the `U:\cxx` folder, then this command:
    ...
    new_project test app U:\cxx\compilers\vs2019
...
-   puts the Visual Studio project file into `name\compilers\vs2019\static\build\name`; and

-   puts the solution file into `name\compilers\vs2019\static\build`.
...
2.  Copy your "data" dir from:<br/>`imported_projects\src\algo\cobalt\unit_test\data`<br/>to:<br/>`imported_projects\compilers\vs2019\static\build\algo\cobalt\unit_test\data`
